...
```css
:root {
  /* 沿用 JP¥ ONLINE 的紙感底層 —— 米白對「年鑑」比深色更對味 */
  --paper:       #f6f3ec;
  --paper-alt:   #ebe7dc;
  --paper-white: #ffffff;
  --ink:         #141414;
  ...
  /* ★ 待定：換掉日系印章朱色，選一個本站識別色 */
  --accent:      /* TBD */;
  --accent-ink:  /* TBD，accent 的加深版，按壓態用 */;
}
```

**待決事項：** accent 色。朱色 `#B8292B` 是日系印章語彙，不適合台灣 AI 影視。選色約束：
- 在 `--paper #F6F3EC` 上對比 ≥ 4.5:1（WCAG AA，18px 以上文字）
- 不可與作品縮圖的全彩互搶——建議偏冷或偏中性
- 需同時提供加深版 `--accent-ink` 給按壓態
...
| # | 項目 | 說明 |
|---|---|---|
| 1 | **accent 色** | 見 §5-3 選色約束。需同時定 `--accent` 與 `--accent-ink` |
